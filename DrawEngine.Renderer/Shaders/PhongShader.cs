...
namespace DrawEngine.Renderer.Shaders {
    ...
    public class PhongShader : Shader {
        ...
        public static Vector3D BumpNormal(Texture bumpTexture, Intersection intersection) {
            ...
            double dx = 1.0d / (bumpTexture.Width - 1);
            double dy = 1.0d / (bumpTexture.Height - 1);
            double b0 = bumpTexture.GetPixel(uv).Luminance;
            double bx = bumpTexture.GetPixel(uv.U + dx, uv.V).Luminance;
            double by = bumpTexture.GetPixel(uv.U, uv.V + dy).Luminance;
                        ...
                        if(material.IsReflective) {
                            ...
                            if(this.RV > 0) {
                                ...
                                color += (material.KSpec * material.SpecularColor * light.Color
                                          * Math.Pow(this.RV, material.Shiness)) * this.lightFactor
                                         * shadowFactor;
                            }
                        }
