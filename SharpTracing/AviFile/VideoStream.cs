...
namespace AviFile
{
    public class VideoStream : AviStream
    {
                           ...
                           PixelFormat format)
        {
            ...
            this.width = width;
            this.height = height;
            this.countBitsPerPixel = this.ConvertPixelFormatToBitCount(format);
            this.firstFrame = 0;
            this.CreateStream();
        }
        ...
        {
            this.Initialize(aviFile, writeCompressed, frameRate, firstFrame);
            this.CreateStream();
            this.AddFrame(firstFrame);
        }
        /// <summary>Initialize a new VideoStream and add the first frame</summary>
        /// <param name="aviFile">The file that contains the stream</param>
        /// <param name="writeCompressed">true: create a compressed stream before adding frames</param>
        ...
        {
            this.Initialize(aviFile, true, frameRate, firstFrame);
            this.CreateStream(compressOptions);
            this.AddFrame(firstFrame);
        }
        ...
        internal VideoStream(int frameSize, double frameRate, int width, int height, Int16 countBitsPerPixel,
        ...
        {
            get { return this.countBitsPerPixel; }
        }
        public int CountFrames
        {
            get { return this.countFrames; }
        }
        ...
        {
            get { return this.compressOptions; }
        }
        public Avi.AVISTREAMINFO StreamInfo
        {
            get { return this.GetStreamInfo(this.aviStream); }
        }
        ...
        private void Initialize(int aviFile, bool writeCompressed, double frameRate, Bitmap firstFrameBitmap)
        {
            ...
            this.firstFrame = 0;
            BitmapData bmpData =
                    firstFrameBitmap.LockBits(new Rectangle(0, 0, firstFrameBitmap.Width, firstFrameBitmap.Height),
            ...
            this.countBitsPerPixel = this.ConvertPixelFormatToBitCount(firstFrameBitmap.PixelFormat);
        ...
        {
            int scale = 1;
            double rate = this.frameRate;
            this.GetRateAndScale(ref rate, ref scale);
            ...
            strhdr.dwSuggestedBufferSize = this.frameSize; //height_ * stride_;
            strhdr.dwQuality = -1; //default
            strhdr.dwSampleSize = 0;
            strhdr.rcFrame.top = 0;
            strhdr.rcFrame.left = 0;
            strhdr.rcFrame.bottom = (uint)this.height;
            strhdr.rcFrame.right = (uint)this.width;
            strhdr.dwEditCount = 0;
            strhdr.dwFormatChangeCount = 0;
            strhdr.szName = new UInt16[64];
            int result = Avi.AVIFileCreateStream(this.aviFile, out this.aviStream, ref strhdr);
            if(result != 0){
        ...
        {
            this.CreateStreamWithoutFormat();
            if(this.writeCompressed){
                this.CreateCompressedStream();
            } else{
                this.SetFormat(this.aviStream);
            }
        }
        /// <summary>Create a new stream</summary>
        ...
        {
            this.CreateStreamWithoutFormat();
            this.CreateCompressedStream(options);
        }
        ...
        private void CreateCompressedStream()
        {
            ...
            Avi.AVISaveOptions(IntPtr.Zero, Avi.ICMF_CHOOSE_KEYFRAME | Avi.ICMF_CHOOSE_DATARATE, 1, ref this.aviStream,
                               ref options);
            ...
            int result = Avi.AVIMakeCompressedStream(out this.compressedStream, this.aviStream, ref this.compressOptions,
                                                     0);
            if(result != 0){
                throw new Exception("Exception in AVIMakeCompressedStream: " + result.ToString());
            }
            this.SetFormat(this.compressedStream);
        }
        /// <summary>Create a compressed stream from an uncompressed stream</summary>
        private void CreateCompressedStream(Avi.AVICOMPRESSOPTIONS options)
        {
            int result = Avi.AVIMakeCompressedStream(out this.compressedStream, this.aviStream, ref options, 0);
            if(result != 0){
                throw new Exception("Exception in AVIMakeCompressedStream: " + result.ToString());
            }
        ...
        public void AddFrame(Bitmap bmp)
        {
            ...
            BitmapData bmpDat = bmp.LockBits(new Rectangle(0, 0, bmp.Width, bmp.Height), ImageLockMode.ReadOnly,
                                             bmp.PixelFormat);
            int result = Avi.AVIStreamWrite(this.writeCompressed ? this.compressedStream : this.StreamPointer,
                                            this.countFrames, 1, bmpDat.Scan0, (Int32)(bmpDat.Stride * bmpDat.Height), 0,
                                            0, 0);
            if(result != 0){
        ...
        public void GetFrameOpen()
        {
            ...
            //Open frames
            Avi.BITMAPINFOHEADER bih = new Avi.BITMAPINFOHEADER();
            bih.biBitCount = this.countBitsPerPixel;
            ...
            this.getFrameObject = Avi.AVIStreamGetFrameOpen(this.StreamPointer, ref bih);
            if(this.getFrameObject == 0){
        ...
        {
            Bitmap bmp = this.GetBitmap(position);
        ...
        {
            if(this.getFrameObject != 0){
                Avi.AVIStreamGetFrameClose(this.getFrameObject);
                this.getFrameObject = 0;
            }
        }
        ...
        public override void ExportStream(String fileName)
        {
            ...
            IntPtr streamPointer = this.StreamPointer;
