...
namespace DrawEngine.Renderer.Mathematics.Algebra {
    public static class EquationSolver {
        ...
        public static int SolveCubic(float a, float b, float c, float d, out float root1, out float root2,
            ...
            const double oneDiv3 = 1.0 / 3.0;
            double p = (float) (oneDiv3 * (-oneDiv3 * sqA + B));
            double q = (float) (0.5 * (2.0 / 27.0 * A * sqA - oneDiv3 * A * B + C));
            ...
            if (D.NearZero()) {
                if (q.NearZero()) {
                    ...
                    float u = (float) Math.Pow(-q, oneDiv3);
                    root1 = u + u;
                    root2 = -u;
                    //if (root1 > root2) {
                    //    float temp = root1;
                ...
                double phi = (oneDiv3 * Math.Acos((-q / Math.Sqrt(-cubeOfP))));
                double t = (2.0 * Math.Sqrt(-p));
                root1 = (float) (t * Math.Cos(phi));
                root2 = (float) (-t * Math.Cos(phi + Math.PI * oneDiv3));
                root3 = (float) (-t * Math.Cos(phi - Math.PI * oneDiv3));
                //if (root2 > root3) {
                //    float temp = root2;
                //    root2 = root3;
                //    root3 = temp;
                ...
                double u = Math.Pow(sqrtD + Math.Abs(q), oneDiv3);
                if (q > 0.0f) {
        ...
        public static int SolveQuartic(float a, float b, float c, float d, float e, out float root1, out float root2,
            ...
            double RIm, DRe, DIm, dum1, ERe, EIm;
            double y1, z1Re, z1Im, z2Re;
            double cb = -c;
            double cc = -4.0 * e + d * b;
            double cd = -(b * b * e + d * d) + 4.0 * c * e;
            //if (cd == 0) alert("cd = 0.");
            // Solve the resolvant cubic for y1
            double q = (3.0 * cc - (cb * cb)) * (1.0 / 9.0);
            double r = -(27.0 * cd) + cb * (9.0 * cc - 2.0 * (cb * cb));
            r *= 1.0 / 54.0;
            double discrim = q * q * q + r * r;
            const double oneDiv3 = 1.0 / 3.0;
            double term1 = (cb * oneDiv3);
            if (discrim > 0.0) {
                // one root real, two are complex
                double s = r + Math.Sqrt(discrim);
                s = ((s < 0.0) ? -Math.Pow(-s, oneDiv3) : Math.Pow(s, oneDiv3));
                double t = r - Math.Sqrt(discrim);
                t = ((t < 0.0) ? -Math.Pow(-t, oneDiv3) : Math.Pow(t, oneDiv3));
                y1 = -term1 + s + t;
            } // End if (discrim > 0)
            else {
                double r13;
                if (discrim.NearZero()) {
                    r13 = ((r < 0.0) ? -Math.Pow(-r, oneDiv3) : Math.Pow(r, oneDiv3));
                    y1 = -term1 + 2.0 * r13;
                } // End if (discrim == 0)
                else {
                    // else discrim < 0
        ...
        public static float[] SolveQuadric(float a, float b, float c) {
            ...
            float t1 = c / q;
            // return sorted array
            return (t0 > t1) ? new[] {t1, t0} : new[] {t0, t1};
        }
