...
namespace Alsing.Design
{
    ...
    public class CollectionEditorGui : UserControl
    {
        ...
        public void Bind()
        {
            var e = this.EditValue as ICollection;
            if(e == null){
        ...
        private void lstMembers_DrawItem(object sender, DrawItemEventArgs e)
        {
            try{
                ...
                int c = this.lstMembers.Items.Count;
                SizeF s = e.Graphics.MeasureString(c.ToString(), this.lstMembers.Font);
                var maxwidth = (int)s.Width;
                if(maxwidth < 16 + 2){
                    maxwidth = 16 + 2;
                }
                var r = new Rectangle(0, e.Bounds.Top, maxwidth, this.lstMembers.ItemHeight);
                bool SupportPaint = this.Editor.GetPaintValueSupported();
                int w = SupportPaint ? 20 : 0;
                var rcItem = new Rectangle(r.Right + w, r.Top, e.Bounds.Width - r.Right - w, this.lstMembers.ItemHeight);
                ControlPaint.DrawBorder3D(e.Graphics, r, Border3DStyle.Raised, Border3DSide.All);
                StringFormat sf = StringFormat.GenericDefault;
                sf.Alignment = StringAlignment.Far;
                r.Inflate(-1, -1);
                e.Graphics.DrawString(e.Index.ToString(), this.lstMembers.Font, Brushes.Black, r, sf);
                bool Selected = ((int)e.State & (int)DrawItemState.Selected) != 0;
                using(SolidBrush bg = this.GetBgBrush(Selected)){
                    using(SolidBrush fg = this.GetFgBrush(Selected)){
                        ...
                        if(e.Index >= 0){
                            object o = this.lstMembers.Items[e.Index];
                            string name = GetDisplayText(o);
                            e.Graphics.DrawString(name, this.lstMembers.Font, fg, rcItem);
                        }
                    }
                }
            } catch{}
        ...
        {
            SolidBrush fg = Selected
                                    ? new SolidBrush(SystemColors.HighlightText)
                                    : new SolidBrush(this.lstMembers.ForeColor);
            return fg;
        }
        private SolidBrush GetBgBrush(bool Selected)
        {
            SolidBrush bg = Selected
                                    ? new SolidBrush(SystemColors.Highlight)
                                    : new SolidBrush(this.lstMembers.BackColor);
            return bg;
        }
        ...
        {
            this.pygProperties.SelectedObject = this.lstMembers.SelectedIndex >= 0 ? this.lstMembers.SelectedItem : null;
        }
        private void EnableRemove()
        {
            this.btnRemove.Enabled = this.lstMembers.SelectedIndices.Count > 0;
        }
