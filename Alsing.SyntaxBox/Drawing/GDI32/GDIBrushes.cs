...
namespace Alsing.Drawing.GDI
{
    ...
    public class GDIBrush : GDIObject
    {
        ...
        {
            this.hBrush = NativeMethods.CreateSolidBrush(NativeMethods.ColorToInt(color));
            this.Create();
        }
        public GDIBrush(Bitmap pattern)
        {
            this.hBrush = NativeMethods.CreatePatternBrush(pattern.GetHbitmap());
            this.Create();
        }
        public GDIBrush(IntPtr hBMP_Pattern)
        {
            this.hBrush = NativeMethods.CreatePatternBrush(hBMP_Pattern);
            //if (hBrush==(IntPtr)0)
        ...
        {
            this.hBrush = NativeMethods.CreateHatchBrush(Style, NativeMethods.ColorToInt(color));
            this.Create();
        }
        public GDIBrush(int BrushIndex)
        {
            this.hBrush = (IntPtr)BrushIndex;
            this.mSystemBrush = true;
            this.Create();
        }
        protected override void Destroy()
        {
            //only destroy if brush is created by us
            if(!this.mSystemBrush){
                if(this.hBrush != (IntPtr)0){
                    NativeMethods.DeleteObject(this.hBrush);
                }
            }
            base.Destroy();
            this.hBrush = (IntPtr)0;
        }
    }
