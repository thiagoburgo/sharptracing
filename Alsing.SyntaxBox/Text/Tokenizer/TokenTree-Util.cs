...
namespace Alsing.Text
{
    public partial class TokenTree
    {
        private void AddCaseInsensitiveToken(string text, bool needSeparators, object[] tags)
        {
            ...
            if(this.nodes[startIndex] == null){
                this.nodes[startIndex] = new TokenTreeNode();
            }
            this.nodes[startIndex].AddToken(text, false, needSeparators, tags);
        ...
        private void AddCaseSensitiveToken(string text, bool needSeparators, object[] tags)
        {
            ...
            this.nodes[startIndex].AddToken(text, true, needSeparators, tags);
        }
                                                         ...
                                                         object[] tags)
        {
            ...
            this.nodes[startIndex].AddPattern(prefix, false, needSeparators, matcher, tags);
            //make a lowercase string with a uppercase start char and add it as a token
                                                       ...
                                                       object[] tags)
        {
            ...
            this.nodes[startIndex].AddPattern(prefix, true, needSeparators, matcher, tags);
        }
                                             ...
                                             object[] tags)
        {
            if(matcher.DefaultPrefixes != null){
                foreach(string defaultPrefix in matcher.DefaultPrefixes){
                    this.AddPattern(defaultPrefix, matcher, caseSensitive, needSeparators, tags);
                }
            } else{
                var patternMatcherReference = new PatternMatchReference(matcher){
                                                                                        Tags = tags,
                                                                                        NextSibling = this.root.FirstExpression,
                                                                                        NeedSeparators = needSeparators
                                                                                };
                this.root.FirstExpression = patternMatcherReference;
            }
        }
    }
}