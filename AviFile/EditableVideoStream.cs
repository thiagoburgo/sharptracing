...
namespace AviFile
{
    public class EditableVideoStream : VideoStream
    {
        ...
        public EditableVideoStream(VideoStream stream)
                : base(
                        stream.FrameSize, stream.FrameRate, stream.Width, stream.Height, stream.CountBitsPerPixel,
                        stream.CountFrames, stream.CompressOptions, stream.WriteCompressed)
        {
            Avi.AVIFileInit();
            int result = Avi.CreateEditableStream(ref this.editableStream, stream.StreamPointer);
            if(result != 0){
                throw new Exception("Exception in CreateEditableStream: " + result.ToString());
            }
            this.SetInfo(stream.StreamInfo);
        }
        /// <summary>Pointer to the unmanaged AVI stream</summary>
        internal override IntPtr StreamPointer
        {
            get { return this.editableStream; }
        }
        ...
        public IntPtr Copy(int start, int length)
        {
            ...
            int result = Avi.EditStreamCopy(this.editableStream, ref start, ref length, ref copyPointer);
            if(result != 0){
        ...
        {
            IntPtr copyPointer = IntPtr.Zero;
            int result = Avi.EditStreamCut(this.editableStream, ref start, ref length, ref copyPointer);
            if(result != 0){
                throw new Exception("Exception in Cut: " + result.ToString());
            }
            this.countFrames -= length;
            return copyPointer;
        }
