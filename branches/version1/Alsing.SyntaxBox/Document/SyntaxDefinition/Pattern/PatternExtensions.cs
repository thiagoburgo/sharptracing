...
namespace Alsing.SourceCode
{
    public partial class Pattern
    {
        ...
        {
            return (this.CharIsSeparator(text, position - 1)
                    && this.CharIsSeparator(text, position + this.StringPattern.Length));
        }
        ...
        private PatternScanResult SimpleFind(string text, int startPosition, bool matchCase)
        {
            int Position = matchCase
                                   ? text.IndexOf(this.StringPattern, startPosition)
                                   : text.ToLowerInvariant().IndexOf(this.LowerStringPattern, startPosition);
