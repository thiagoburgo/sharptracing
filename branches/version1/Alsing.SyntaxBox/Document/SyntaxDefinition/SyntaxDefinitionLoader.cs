...
namespace Alsing.SourceCode
{
    ...
    public class SyntaxDefinitionLoader
    {
        ...
        {
            this.styleLookup = new Hashtable();
            this.spanDefinitionLookup = new Hashtable();
            this.syntaxDefinition = new SyntaxDefinition();
            var xmlDocument = new XmlDocument();
            xmlDocument.Load(File);
            this.ReadLanguageDefinition(xmlDocument);
        ...
        public SyntaxDefinition Load(string File, string Separators)
        {
            this.styleLookup = new Hashtable();
            this.spanDefinitionLookup = new Hashtable();
            this.syntaxDefinition = new SyntaxDefinition{Separators = Separators};
            var xmlDocument = new XmlDocument();
            xmlDocument.Load(File);
            this.ReadLanguageDefinition(xmlDocument);
            if(this.syntaxDefinition.mainSpanDefinition == null){
        ...
        {
            this.ParseLanguage(xml["Language"]);
        }
        private void ParseLanguage(XmlNode node)
        {
            ...
            this.syntaxDefinition.Name = Name;
            this.syntaxDefinition.mainSpanDefinition = this.GetBlock(StartBlock);
            foreach(XmlNode n in node.ChildNodes){
                if(n.NodeType == XmlNodeType.Element){
                    if(n.Name.ToLowerInvariant() == "filetypes"){
                        this.ParseFileTypes(n);
                    }
            ...
            foreach(XmlNode n in node.ChildNodes){
                if(n.NodeType == XmlNodeType.Element){
                    if(n.Name.ToLowerInvariant() == "filetype"){
                        ...
                        this.syntaxDefinition.FileTypes.Add(ft);
                    }
                }
            }
        }
        private void ParseBlock(XmlNode node)
        {
            ...
            SpanDefinition bl = this.GetBlock(Name);
            bl.BackColor = BackColor;
            bl.Name = Name;
            bl.MultiLine = IsMultiline;
            bl.Style = this.GetStyle(Style);
            ...
            foreach(XmlNode n in node.ChildNodes){
                if(n.NodeType == XmlNodeType.Element){
                    if(n.Name.ToLowerInvariant() == "scope"){
                        ...
                        if(Start != ""){
                            ...
                            Pattern endPattern = EscapeChar != ""
                                                         ...
                                                         : new Pattern(End, blnEndIsComplex, false,
                            ...
                            if(spawnStart != ""){
                                scope.spawnSpanOnStart = this.GetBlock(spawnStart);
                            }
                            if(spawnEnd != ""){
                                scope.spawnSpanOnEnd = this.GetBlock(spawnEnd);
                            }
                        }
                    }
                    if(n.Name.ToLowerInvariant() == "bracket"){
                        ...
                        if(Start != ""){
                            var pl = new PatternList{Style = this.GetStyle(style)};
                            bool blnStartIsComplex = bool.Parse(StartIsComplex);
                            bool blnEndIsComplex = bool.Parse(EndIsComplex);
                    ...
                    foreach(XmlNode cn in n.ChildNodes){
                        if(cn.Name.ToLowerInvariant() == "patterngroup"){
                            ...
                            foreach(XmlAttribute att in cn.Attributes){
                                switch(att.Name.ToLowerInvariant()){
                                    case "style":
                                        pl.Style = this.GetStyle(att.Value);
                                        break;
                                    case "name":
                                        pl.Name = att.Value;
                                        break;
                ...
                if(n.Name.ToLowerInvariant() == "childblocks"){
                    foreach(XmlNode cn in n.ChildNodes){
                        if(cn.Name.ToLowerInvariant() == "child"){
                            foreach(XmlAttribute att in cn.Attributes){
                                if(att.Name.ToLowerInvariant() == "name"){
                                    bl.childSpanDefinitions.Add(this.GetBlock(att.Value));
                                }
                            }
                        }
                    }
        ...
        {
            if(this.styleLookup[Name] == null){
                var s = new TextStyle();
                this.styleLookup.Add(Name, s);
            }
            return (TextStyle)this.styleLookup[Name];
        }
        //done
        private SpanDefinition GetBlock(string Name)
        {
            if(this.spanDefinitionLookup[Name] == null){
                var b = new SpanDefinition(this.syntaxDefinition);
                this.spanDefinitionLookup.Add(Name, b);
            }
            return (SpanDefinition)this.spanDefinitionLookup[Name];
        }
        //done
        private void ParseStyle(XmlNode node)
        {
            ...
            TextStyle st = this.GetStyle(Name);
            if(BackColor != ""){
