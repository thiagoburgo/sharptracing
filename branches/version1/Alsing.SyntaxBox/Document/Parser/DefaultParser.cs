...
namespace Alsing.SourceCode.SyntaxDocumentParsers
{
    ...
    public sealed class DefaultParser : IParser
    {
        ...
        {
            get { return this.mSyntaxDefinition; }
            set
            {
                this.mSyntaxDefinition = value;
                if(this.mSyntaxDefinition == null){
                    var l = new SyntaxDefinition();
                    l.mainSpanDefinition = new SpanDefinition(l){MultiLine = true};
                    this.mSyntaxDefinition = l;
                }
                this.Version = long.MinValue;
                this.mSyntaxDefinition.Version = long.MinValue + 1;
        ...
        {
            this.InternalParseLine(index, ParseKeywords);
            if(this.SyntaxDefinition != null){
                if(this.Version != this.SyntaxDefinition.Version){
                    this.SyntaxDefinition.UpdateLists();
                    this.Version = this.SyntaxDefinition.Version;
                }
            }
        ...
        {
            get { return this.SyntaxDefinition.Separators; }
            set { this.SyntaxDefinition.Separators = value; }
        }
        #endregion
        ...
        public void Init(string SyntaxFile)
        {
            try{
                ...
                this.SyntaxDefinition = new SyntaxDefinitionLoader().Load(SyntaxFile);
            } catch{}
        }
        public void Init(string syntaxFile, string separators)
        {
            try{
                ...
                this.SyntaxDefinition = new SyntaxDefinitionLoader().Load(syntaxFile, separators);
            } catch{}
        }
        /// <summary>
        /// 
        ...
        private void MakeSame(int RowIndex)
        {
            ...
            Span seg2 = this.Document[RowIndex + 1].startSpan;
            while(seg != null){
        ...
        private bool IsSameButDifferent(int RowIndex, Span oldStartSpan)
        {
            ...
            Row row = this.Document[RowIndex];
            Span seg = row.endSpan;
            Span oldEndSpan = this.Document[RowIndex + 1].startSpan;
        ...
        private ScanResultWord GetNextWord(string Text, Span currentSpan, int StartPos, ref bool HasComplex)
        {
            ...
            if(HasComplex){
                foreach(Pattern pattern in spanDefinition.ComplexPatterns){
                    PatternScanResult scanres = pattern.IndexIn(Text, StartPos, pattern.Parent.CaseSensitive,
                                                                this.Separators);
                    if(scanres.Token != ""){
        ...
        private void ParseText(Row Row, Span currentSpan, string Text)
        {
            ...
            while(true){
                ScanResultWord Word = this.GetNextWord(Text, currentSpan, CurrentPosition, ref HasComplex);
                if(!Word.HasContent){
        ...
        private void InternalParseLine(int index, bool ParseKeywords)
        {
            ...
            //				return;
            //			ParseKeywords=true;
            SyntaxDocument doc = this.Document;
            Row Row = doc[index];
            ...
            if(index > 0){
                Row.startSpan = this.Document[index - 1].endSpan;
            } else{
                if(Row.startSpan == null){
                    Row.startSpan = new Span(Row){spanDefinition = this.mSyntaxDefinition.mainSpanDefinition};
                }
            }
            int CurrentPosition = 0;
            ...
            while(true){
                ScanResultSegment ChildSegment = this.GetNextChildSegment(Row, currentSpan, CurrentPosition);
                ScanResultSegment EndSegment = this.GetEndSegment(Row, currentSpan, CurrentPosition);
                if((EndSegment.HasContent && ChildSegment.HasContent && EndSegment.Position <= ChildSegment.Position)
                   || (EndSegment.HasContent && ChildSegment.HasContent == false)){
                    ...
                    if(ParseKeywords){
                        ...
                        this.ParseText(Row, currentSpan, Text);
                    }
            ...
            if(this.IsSameButDifferent(index, oldStartSpan)){
                this.MakeSame(index);
            ...
            if(!ParseKeywords){
                if(oldEndSpan != null){
                    if(Row.endSpan != oldEndSpan && index <= this.Document.Count - 2){
                        //if (Row.CanFold)
                        //	Row.expansion_StartSpan.Expanded = true;
                        this.Document[index + 1].AddToParseQueue();
                        this.Document.NeedResetRows = true;
                    }
                } else if(index <= this.Document.Count - 2){
                    //if (Row.CanFold)
                    //	Row.expansion_StartSpan.Expanded = true;
                    this.Document[index + 1].AddToParseQueue();
                    this.Document.NeedResetRows = true;
            ...
            while(seg != null){
                if(seg == currentSpan || seg.spanDefinition.TerminateChildren){
                    foreach(Pattern end in seg.Scope.EndPatterns){
                        PatternScanResult psr = end.IndexIn(Row.Text, StartPos, seg.Scope.CaseSensitive, this.Separators);
        ...
        private ScanResultSegment GetNextChildSegment(Row Row, Span currentSpan, int StartPos)
        {
            ...
            foreach(SpanDefinition ChildBlock in currentSpan.spanDefinition.childSpanDefinitions){
                ...
                foreach(Scope Scope in ChildBlock.ScopePatterns){
                    PatternScanResult psr = Scope.Start.IndexIn(Row.Text, StartPos, Scope.CaseSensitive, this.Separators);
                    int CurrentPosition = psr.Index;
