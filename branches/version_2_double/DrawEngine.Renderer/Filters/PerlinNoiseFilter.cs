...
namespace DrawEngine.Renderer.Filters
{
    public static class PerlinNoiseFilter
    {
        private readonly static int[] p = new int[512];
        private readonly static int[] permutation = {
                                                            ...
                                                            22, 39, 253, 19, 98, 108, 110, 79, 113, 224, 232, 178, 185, 112,
        ...
        public static double Stripes(double x, double f)
        {
            double t = .5 + .5 * Math.Sin(f * 2 * Math.PI * x);
            return t * t - .5;
        }
        // TURBULENCE TEXTURE
        public static double Turbulence(double x, double y, double z, double width, double freq)
        {
            double t = -.5;
        ...
        // NOISE TEXTURE
        public static double Noise(double x, double y, double z, double freq)
        {
            double x1, y1, z1;
            x1 = .707 * x - .707 * z;
            z1 = .707 * x + .707 * z;
            y1 = .707 * x1 + .707 * y;
            x1 = .707 * x1 - .707 * y;
            return Noise(freq * x1 + 100, freq * y1, freq * z1);
        }
        public static double Stripes2(double x, double f)
        {
            double t = .15 + .15 * Math.Abs(Math.Cos(f * 2 * Math.PI * x)) + .7 * Math.Cos(f * 2 * Math.PI * x);
            return t * t - .15;
        }
        public static double Stripes3(double x, double f)
        {
            double t = .5 + .5 * Math.Abs(Math.Sin(f * 2 * Math.PI * x));
            return t * t - .5;
        }
    }
}